import numpy as np
import torch
from cnn_doa_M20 import CNN_doa, Conv1, Conv2, Conv3
from datatest import Data_Loader
...
model1 = torch.load("example_SNR-1019.pth")
data_dataset = Data_Loader('./CNN_DOA/CNN_SNR-10.mat')
...
for data in train_dataloader:
    ...
    #     k += 1
    #     reg_c = e[i, ::].cpu().detach().numpy()  # 因为output_c的batchsize是32，所以说包含着32个一维向量，相当于包含着32个方位普
    ...
    #         c_second = c_max
    #         c_max = c
    #     position_max = c_max - 90  # C_max是方位谱峰值在数组中的位置，数组的索引从0开始，到120结束，索引的向量位置减去60
    #     # 得到的就是目标粗略的角度，加上误差就是得到准确的误差
    #     position_second = c_second - 90
    ...
    #         label_max[0, 0] = c1
    #     mask_max = label_max[0, 0] - 90  # 减60而不是61的原因是python首字母第一个元素位置为0，而不是1
    #     mask_second = label_max[0, 1] - 90
